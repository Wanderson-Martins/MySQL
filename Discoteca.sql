...
CREATE TABLE gravadora (
	id_gravadora INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE disco (
    ...
    titulo_disco VARCHAR(90) NOT NULL,
...
-- Realizando criação das Views 

CREATE VIEW v_disco_artista_genero AS 
SELECT d.id_disco, d.titulo_disco, a.nome_artista, g.nome_genero FROM disco d
...
SELECT d.titulo_disco, m.nome_musica, m.tempo_musica FROM disco d
INNER JOIN musica m ON d.id_disco = m.id_disco;

CREATE VIEW v_gravadora_quantidade_discos AS 
SELECT g.nome_gravadora, COUNT(d.id_disco) AS quantidade_discos FROM gravadora g
LEFT JOIN disco d ON g.id_gravadora = d.id_gravadora
GROUP BY g.nome_gravadora;

-- Chamando as views
-- 1ª
select * from v_disco_artista_genero;

-- 2ª
select * from v_disco_musica;

-- 3ª
select * from v_gravadora_quantidade_discos;
...
BEGIN
    ...
    SELECT TIMESTAMPDIFF(YEAR, dt_nasc_artista, CURDATE()) INTO @age FROM artista WHERE id_artista = p_id_artista;
    IF @age < 12 THEN
    ...
    IF has_error = FALSE THEN
        INSERT INTO disco (titulo_disco, tempo_disco, ano_lancamento, id_artista, id_gravadora, id_genero) 
...
UPDATE artista SET nome_artista = nome_artista, dt_nasc_artista = dt_nasc_artista 
...
drop procedure sp_update_disco;
-- Chamando as Procedures

-- 1ª
CALL sp_insert_disco('Teste', 120.5, 2023, 1, 1, 1);

-- 2ª
CALL sp_delete_musica(1);

-- 3ª
CALL sp_update_artista(5, 'John Lennon', '1940-09-10');
